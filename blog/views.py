...
from .models import Article, Tag
from .serializers import ArticleListSerializer, ArticleDetailSerializer, TagSerializer
...
class ArticleViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    # اتصال بک‌اند‌های لازم
    filter_backends = [DjangoFilterBackend, filters.SearchFilter, filters.OrderingFilter]
    
    # کلاس فیلتر سفارشی ما
    filterset_class = ArticleFilter
    
    # فیلدهایی که جستجو روی آن‌ها اعمال می‌شود
    search_fields = ['title', 'content']
    
    # فیلدهایی که برای مرتب‌سازی مجاز هستند
...
class TagViewSet(viewsets.ReadOnlyModelViewSet):
    """ViewSet فقط-خواندنی برای تگ‌ها."""
    queryset = Tag.objects.all()
    serializer_class = TagSerializer
    permission_classes = [permissions.AllowAny]